# custom-javascript-functions-list

## Endpoint
- Method: `post`
- Path: `/api/v1/{tenant_id}/custom-javascript-functions/list`
...
- fields:
  ...
  - requestBody (application/json).sort_keys
    - type: ['array', 'null']
    - required: no
    - description: The keys to sort the functions by. Valid columns: name, created_at, updated_at. Default: created_at DESC
  - requestBody (application/json).sort_keys[]
    - type: object
    - required: no
    - description: The keys to sort the functions by. Valid columns: name, created_at, updated_at. Default: created_at DESC
...
- 200
  - description: OK
  - content-type: application/json
  - schema: CustomJavascriptFunctionsListOutputBody
