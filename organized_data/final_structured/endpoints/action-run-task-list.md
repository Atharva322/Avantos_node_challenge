# action-run-task-list

## Endpoint
- Method: `post`
- Path: `/api/v1/{tenant_id}/tasks`
...
- fields:
  ...
  - requestBody (application/json).filter
    - type: string
    - required: no
    - description: The Expression to filter the tasks by. See the Readme for more details on the Expression format.
  - requestBody (application/json).include_priorities
    - type: boolean
    - required: no
    - description: Whether to include task priorities from the blueprint version configuration
  - requestBody (application/json).only_assigned
    - type: boolean
    - required: no
    - description: Whether to only return tasks for client organisations the current user is directly assigned to
  ...
  - requestBody (application/json).sort_keys
    - type: ['array', 'null']
    - required: no
    - description: The keys to sort the tasks by.
  - requestBody (application/json).sort_keys[]
    - type: object
    - required: no
    - description: The keys to sort the tasks by.
  ...
  - requestBody (application/json).source_system_filter
    - type: string
    - required: no
    - description: Filter tasks by source system
  - requestBody (application/json).use_enriched_unqork_tasks
    - type: boolean
    - required: no
    - description: Flag that decides if the unqork data will have additional informations, such as action runs extra data

## Responses
- 200
  - description: OK
  - content-type: application/json
  - schema: ActionRunTaskListOutputBody
